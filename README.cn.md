...
每条请求走最优路径，尽量避免不必要的 LLM 调用。

- **五种路由类型**：`[TALK]`（约 70%）→ SoloRunner 直通；`[DIRECT]` → 跳过规划；`[REFRAME]` → 意图重加工；`[SCHEDULE]` → 定时执行；`[BLOCK]` → 安全拦截。
- **零延迟静态规则（0ms）**：内置影视触发词（17 个）和软件触发词（12 个）。`clean_target()` 自动剔除修饰语、精准提取核心实体，绝大多数高频请求零 LLM 调用完成分流。
- **动态短路路由**：命中关键词（如 `resource-downloader`）后完全绕过 Strategist，直接路由到工具执行。内置可信域白名单（15 个）和流氓站黑名单（13 个）自动过滤。
- **LLM 语义兜底**：仅在静态规则未命中时启用。若 LLM 判断消息不应走 REFRAME，返回 `REDIRECT` 重新分流。规则引擎覆盖约 80% 的高频请求，成本极低。

### 2. 全维度安全沙箱与隐私隔离

纵深防御体系从网络边界延伸至 LLM 调用层，所有拦截支持平滑降级，不阻塞用户流程。

**数据隐私漏斗（数据不出本机原则）**

| 层级 | 机制 | 延迟 |
|------|------|------|
| L0 路径截断 | `LOCAL_DIRS` 路径匹配 → 强制本地模型 | 0ms |
...
- **输出注入防护**：扫描工具返回内容（浏览器、文件读取），防止网页恶意指令劫持 Agent。
- **技能供应链检测**：挂载第三方技能包（`SKILL.md`）时自动静态扫描，拦截 eval/base64 混淆、隐藏网络请求、嵌入系统指令。

**运行时沙箱**
- `PathGuard`：基于 `os.path.realpath` 严格前缀校验，防止符号链接和 `../` 目录遍历。
- `StateGuard`：跨进程原子事务锁，消除多智能体并发写入的竞态条件。
- 工具调用限流，防止 LLM 幻觉导致的无限循环。

**边界网关与审计**
- API Key 鉴权 + Webhook HMAC-SHA256 + IP 滑动窗口限流（100 次/分钟）。
- 日志脱敏：API Key、Token 等凭证在写盘前自动不可逆掩码。
- 热更新接口键白名单校验 + 超长值熔断。
...
- **UIA 引擎**：通过系统无障碍 API 读取全屏控件信息（类型、名称、位置、状态），确定性交互，快且稳。
- **YOLO 引擎**：内置 39MB 超轻量检测模型随仓库分发，无需额外下载。完美覆盖 UIA 盲区：自定义控件、游戏图标、非标准 UI。
- **双擎协作**：`desktop_grounding_scan` 负责全域元素感知，`desktop_act` 负责精准点击/输入模拟。UIA 保效率，YOLO 保覆盖率。

### 4. Guardian：自愈自调度的看门狗进程

独立于主进程的外部守护进程——主进程彻底崩溃后仍能重启，实现真正的无人值守运行。
...
| 资源熔断 | 每 15s 采样 CPU/内存 | CPU > 95% 或内存 > 2GB 持续 120s → 强杀 |
| 时间齿轮 | 每 60s 轮询 `schedules.json` | 到期任务精准触发执行 |

**自动故障修复**
- **缺包自装**：捕获 `ModuleNotFoundError` 后从 23 个安全库白名单匹配并自动安装，防止恶意注入。
- **端口自释**：跨平台正则提取占用端口并终止占用进程。
- **服务唤醒**：`aria2c` 等周边服务 RPC 不响应时自动拉起。
...
每轮对话结束后瞬间触发后台扫描，不阻塞当前用户进程。通过本地模型（永不走云端）分析最近 5 轮上下文（每轮截断 200 字符），捕捉三类信号：
...
Executor 完成后，独立 Auditor 进行最终裁决：

| 裁决 | 含义 |
|------|------|
| `AFFIRM` | 审核通过，结果交付 |
| `REMAND` | 质量不达标，静默重做 |
| `REPLAN` | 路径死胡同，Strategist 重新推演 |
| `CLOSURE` | 优雅收尾，任务无法达成 |
| `ESCALATE` | 高危/权限阻断，升级请求人工介入 |

`_robust_json_parse()` 自动修复残缺 LLM 输出（Markdown 误包裹、尾部逗号、中文引号等）。Auditor 超时时安全降级为 `PASS_WITH_WARNING`，绝不因审计系统故障卡死用户流程。
