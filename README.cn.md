...
### 1. Router 智能五分流 + 零延迟规则引擎

每条请求走最优路径，将 LLM 调用压缩到真正必要的场景。

- **五种路由类型**：`[TALK]`（约 70%）→ SoloRunner 直通极速响应；`[DIRECT]` → 短路直达 MissionRunner，跳过规划阶段；`[REFRAME]` → 进入语义重加工链，解析模糊或复杂意图；`[SCHEDULE]` → 解析为定时计划，后台准时触发；`[BLOCK]` → 安全拦截（含下载关键词智能降级为 REFRAME）。
- **零延迟静态规则（0ms）**：内置影视触发词（17 个）和软件触发词（12 个），`clean_target()` 自动剔除"帮我下载""请问""1080p"等修饰语，精准提取核心实体——绝大多数高频请求全程零 LLM 调用。
- **动态短路路由**：命中目标关键字（如 `resource-downloader`）后完全绕过 Strategist，正则解析参数直达工具执行。内置可信域白名单（15 个，含 github.com、microsoft.com 等）和流氓站黑名单（13 个）自动过滤。
- **LLM 语义兜底**：仅在静态规则未命中时启用；若 LLM 判断消息不需走 REFRAME，返回 `REDIRECT` 让 Router 重新分流。规则引擎覆盖约 80% 高频请求，0ms 成本。

### 2. 全维度安全沙箱与隐私隔离

纵深防御体系从网络边界延伸至 LLM 调用层。遵循"宁可漏检也绝不阻塞用户"原则，所有拦截支持平滑降级。

**数据隐私漏斗（数据不出本机原则）**

| 层级 | 机制 | 延迟 |
|------|------|------|
| L0 物理截断 | `LOCAL_DIRS` 路径匹配 → 强制本地模型，从源头切断数据上云 | 0ms |
| L1 PII 扫描 | Microsoft Presidio（中英双语）— 手机（0.8）、身份证（0.85）、银行卡（0.6）| 5–20ms |
| 视觉脱敏 | PaddleOCR 提取屏幕文字 → Presidio 扫描 → 仅安全描述发云端；原图永不离机 | 逐帧 |

**AdvancedGuard 大模型防御**
- **越狱拦截**：三级正则矩阵，实时拦截 DAN 模式、"忽略之前指令"、无限制角色扮演等越狱尝试。
- **输出注入防护**：Agent 使用浏览器或读取外部文件时，实时扫描工具返回内容，防止网页恶意隐藏指令劫持 Agent。
- **技能供应链检测**：挂载第三方技能包（`SKILL.md`）时自动静态扫描，拦截 eval/base64 混淆执行、隐藏网络请求、嵌入系统指令。

**运行时沙箱**
- `PathGuard`：基于 `os.path.realpath` 严格前缀校验，防止符号链接和 `../` 目录遍历攻击。
- `StateGuard`（RSA-Synchronizer）：跨进程原子事务锁，消除多智能体并发写入的竞态条件。
- 工具调用限速：自动中断 LLM 幻觉导致的无限死循环，防止算力和 API 额度意外耗尽。

**边界网关与审计**
- API Key 鉴权 + Webhook HMAC-SHA256 + IP 滑动窗口限流（100 次/分钟）。
- 日志脱敏：API Key、Token 等凭证在落盘前自动不可逆掩码，杜绝外泄。
- 热更新接口键白名单校验 + 超长值熔断，防御缓冲区溢出型攻击。

### 3. UIA 矩阵扫描 + YOLO 视觉接地

无需目标应用提供任何接口——屏幕上看得到，Rooster 就能操控。

- **UIA 引擎**：通过系统无障碍 API 获取全屏所有控件的标准化信息（类型、名称、位置、状态），确定性交互，快且稳。
- **YOLO 引擎**：内置 39MB 超轻量目标检测模型随仓库分发，无需额外下载。完美覆盖 UIA 盲区：自定义控件、游戏图标、非标准 UI 元素。
- **双擎协作**：`desktop_grounding_scan` 负责全域元素感知，`desktop_act` 负责精准点击/输入模拟。UIA 保效率，YOLO 保覆盖率。

### 4. Guardian：自愈、自调度、自修复的守护进程

独立于主进程的外部看门狗——即使主进程彻底崩溃，守护与重启能力不受影响，实现真正的无人值守运行。
...
**自动故障修复**
- **缺包自装**：捕获 `ModuleNotFoundError` 后，从 23 个安全库白名单匹配并自动 `pip install`，白名单机制防止恶意注入。
- **端口自释**：跨平台正则提取占用端口号并精准终止占用进程。
- **服务唤醒**：`aria2c` 等周边服务 RPC 不响应时自动拉起守护进程。

**韧性控制**：熔断器（连续 2 次同类错误停止重试）、重启风暴免疫（300s 内 5 次重启触发告警并停止）、指数退避 + 随机抖动、单实例 PID 互斥、飞书/钉钉/Slack Webhook 告警。

### 5. 双重记忆自进化 + Auditor 质量门禁

**后台自进化（非阻塞）**

每轮对话结束后瞬间触发后台扫描，不阻塞当前用户进程。通过本地模型（永不走云端）分析最近 5 轮上下文（每轮截断 200 字符），捕捉三类信号并写入记忆库：

| 信号 | 触发词示例 | 写入目标 |
|------|-----------|----------|
| `CORRECTION` | "不对""你理解错" | `SOUL.md` → 核心行为原则 |
| `PREFERENCE` | "以后""我希望" | `USER.md` → 用户偏好画像 |
| `MILESTONE` | "已上线""成功了" | `USER.md` → 当前重点项目 |

核心身份字段（Identity / Hard Limits / Memory Protocol）受代码级硬保护，自进化引擎无法篡改。

**Auditor 质量门禁**

Executor 完成后，独立 Auditor 进行最终裁决，五种处置：
...
| `CLOSURE` | 优雅收尾，任务确实无法达成 |
| `ESCALATE` | 高危/权限阻断，升级请求人工介入 |

`_robust_json_parse()` 自动修复残缺 LLM 输出（Markdown 误包裹、尾部逗号、中文引号等）。Auditor 超时时安全降级为 `PASS_WITH_WARNING`，不因审计系统故障卡死用户流程。
